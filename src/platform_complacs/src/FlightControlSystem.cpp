...
namespace platform_complacs
{
    ...
    public:
        ...
        void onInit()
        {
            ...
            if (!this->getPrivateNodeHandle().getParam("rate_max",rate_max))
                rate_max = 100;
            if (!this->getPrivateNodeHandle().getParam("rate_aci",rate_aci))
                rate_aci = 10;
            if (!this->getPrivateNodeHandle().getParam("rate_est",rate_est))
                rate_est = 0;
            if (!this->getPrivateNodeHandle().getParam("rate_imu",rate_imu))
                rate_imu = 0;
            if (!this->getPrivateNodeHandle().getParam("rate_pos",rate_pos))
                rate_pos = 0;
            if (!this->getPrivateNodeHandle().getParam("rate_alt",rate_alt))
                rate_alt = 0;
            if (!this->getPrivateNodeHandle().getParam("rate_inf",rate_inf))
                rate_inf = 0;
            if (!this->getPrivateNodeHandle().getParam("rate_mag",rate_inf))
                rate_inf = 0;
            ...
            if (serial.isOpen())
            {
                ...
                if (rate_aci > 0)
                {
                    ...
                    timerHeartbeat = this->getPrivateNodeHandle().createTimer(
    ...
    PLUGINLIB_EXPORT_CLASS(platform_complacs::FlightControlSystem, nodelet::Nodelet);
